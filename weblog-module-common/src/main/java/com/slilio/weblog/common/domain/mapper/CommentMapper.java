...
public interface CommentMapper extends BaseMapper<CommentDO> {
   ...
   * @return
   */
  default Page<CommentDO> selectPageList(
      Long current,
      Long size,
      String routerUrl,
      LocalDate startDate,
    ...
    // 构建查询对象
    LambdaQueryWrapper<CommentDO> wrapper =
        Wrappers.<CommentDO>lambdaQuery()
            .like(StringUtils.isNotBlank(routerUrl), CommentDO::getRouterUrl, routerUrl) // like模糊查询
            .eq(Objects.nonNull(status), CommentDO::getStatus, status) // 评论状态
            .ge(Objects.nonNull(startDate), CommentDO::getCreateTime, startDate) // 大于等于startDate
            .le(Objects.nonNull(endDate), CommentDO::getCreateTime, endDate) // 小于等于startDate
            .orderByDesc(CommentDO::getCreateTime); // 倒序排列
  ...
  default int deleteByParentCommentId(Long id) {
    return delete(Wrappers.<CommentDO>lambdaQuery().eq(CommentDO::getParentCommentId, id));
  }
}
