...
import com.slilio.weblog.web.model.vo.search.SearchArticlePageListRspVO;
...
public class SearchServiceImpl implements SearchService {
  ...
  public Response searchArticlePageList(SearchArticlePageListReqVO searchArticlePageListReqVO) {
    ...
    // 想要搜索的文档字段（这里指定对文章的标题、摘要进行检索，任何一个字段包含该关键词，都会被搜索到）
        ...
        document -> {
          try {
            ...
            TokenStream tokenStream =
                analyzer.tokenStream(ArticleIndex.COLUMN_TITLE, new StringReader(title));
            String titleFragment = highlighter.getBestFragment(tokenStream, title);

            // 如果没有匹配到关键词，则返回原始文本
            String highlightedTitle =
                StringUtils.isNoneBlank(titleFragment) ? titleFragment : title;
            ...
            // 组转VO
            SearchArticlePageListRspVO vo =
                SearchArticlePageListRspVO.builder()
                    .id(Long.valueOf(id))
